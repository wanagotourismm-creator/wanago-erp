...
function openNewPkgModal() {
  ...
  var _el_pkg_modal_title=document.getElementById('pkg-modal-title');if(_el_pkg_modal_title){_el_pkg_modal_title.textContent='New Package'}
  var _el_pkg_save_btn=document.getElementById('pkg-save-btn');if(_el_pkg_save_btn){_el_pkg_save_btn.textContent='Save Package'}
...
function updatePkgPricePreview() { const price=parseFloat(document.getElementById('pkg-price')?.value);const gst=parseFloat(document.getElementById('pkg-gst')?.value||5);const mrp=parseFloat(document.getElementById('pkg-mrp')?.value);const prev=document.getElementById('pkg-price-preview');if(!prev)return;if(!price){prev.style.display='none';return;} prev.style.display='';const gstAmt=Math.round(price*gst/100);var _el_pp_base=document.getElementById('pp-base');if(_el_pp_base){_el_pp_base.textContent='₹'+price.toLocaleString('en-IN')}var _el_pp_gst=document.getElementById('pp-gst');if(_el_pp_gst){_el_pp_gst.textContent='₹'+gstAmt.toLocaleString('en-IN')}var _el_pp_total=document.getElementById('pp-total');if(_el_pp_total){_el_pp_total.textContent='₹'+(price+gstAmt).toLocaleString('en-IN')}const dw=document.getElementById('pp-discount-wrap');if(mrp&&mrp>price){var _el_pp_discount=document.getElementById('pp-discount');if(_el_pp_discount){_el_pp_discount.textContent=Math.round((1-price/mrp)*100)+'% off MRP'}dw.style.display='';}else{dw.style.display='none';} }
