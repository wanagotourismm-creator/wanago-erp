...
// Same narrow-dot-path-update approach as id-card/route.ts — never calls
// updateClosure (full-section replace, no ownership check). Only writes
// closure.feedback.status/notes, leaving voiceFeedbackUrl (a separate,
// staff-only upload path) untouched.
export async function POST(req: NextRequest) {
  ...
  let body: { opsId?: string; notes?: string };
  try {
    body = await req.json();
  } catch {
    return NextResponse.json({ error: "Invalid request" }, { status: 400 });
  }
  if (!body.opsId || !body.notes?.trim()) return NextResponse.json({ error: "Missing opsId/notes." }, { status: 400 });
  ...
  if (status !== "tour_completed" && status !== "package_closed") {
    return NextResponse.json({ error: "Feedback can only be submitted after your trip is complete." }, { status: 400 });
  }

  await opsRef.update({
    "closure.feedback.status": "collected",
    "closure.feedback.notes": body.notes.trim(),
  ...
  try {
    ...
    await Promise.all(
      opsApprovers.map((u) =>
        notifyUserServer({
          ...
          title: `Feedback received: ${ops.refNumber}`,
          body: `${ops.customerName} submitted trip feedback via the customer portal.`,
    ...
    // Best-effort — the feedback already saved regardless.
  }
