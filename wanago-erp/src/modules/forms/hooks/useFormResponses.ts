...
import type { Form, FormResponse } from "@/modules/forms/types";

export function useFormResponses(form: Form | null) {
  ...
  async function submit(answers: Record<string, string | string[] | number | null>): Promise<{ error: string | null }> {
    if (!form) return { error: "No form selected" };
    try {
      await submitFormResponse(form, {
