...
import { essAssetRequestSchema, type EssAssetRequestSchema } from "@/modules/assets/schemas";
import { ASSET_CATEGORIES } from "@/modules/assets/types";

type Props = {
  open: boolean;
  onClose: () => void;
  onSubmit: (data: EssAssetRequestSchema) => Promise<{ error: string | null }>;
};
...
export function RequestAssetForm({ open, onClose, onSubmit }: Props) {
  const { register, handleSubmit, reset, setError, formState: { errors, isSubmitting } } = useForm<EssAssetRequestSchema>({
    resolver: zodResolver(essAssetRequestSchema),
    defaultValues: { assetCategory: "", reason: "" },
  });

  useEffect(() => {
    if (open) reset({ assetCategory: "", reason: "" });
  }, [open, reset]);

  if (!open) return null;

  async function submit(data: EssAssetRequestSchema) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="modal-enter relative w-full max-w-lg max-h-[90dvh] flex flex-col rounded-2xl border border-primary/20 bg-card shadow-2xl overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-4 scrollbar-thin">
          ...
          <Field label="Asset Category *" error={errors.assetCategory?.message}>
            <select className={inp} {...register("assetCategory")}>
          ...
          <Field label="Reason *" error={errors.reason?.message}>
            <textarea className={`${inp} resize-none`} rows={3} placeholder="Why do you need this..." {...register("reason")} />
          </Field>
        </div>
