...
export function TicketsTable({ tickets, loading, onSetStatus, onAssignToMe, onDelete }: Props) {
              ...
              <tr key={t.id} className="hover:bg-muted/20 transition-colors group">
                ...
                <td className="px-4 py-3">
                  <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button onClick={() => onAssignToMe(t)} title="Assign to me" className="flex h-7 w-7 items-center justify-center rounded-lg text-muted-foreground hover:bg-primary/10 hover:text-primary"><UserCheck size={13} /></button>
                    <button onClick={() => onDelete(t)} title="Delete" className="flex h-7 w-7 items-center justify-center rounded-lg text-muted-foreground hover:bg-destructive/10 hover:text-destructive"><Trash2 size={13} /></button>
                  </div>
                </td>
              </tr>
            ))}
