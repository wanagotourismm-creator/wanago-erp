...
export type AdminTabKey =
  ...
  | "export" | "announcements" | "holidays" | "assets" | "tickets" | "health" | "trash" | "explorer";
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        {stats.map(s => (
          <div key={s.label} className="rounded-2xl border border-white/10 bg-slate-900 p-5 shadow-sm">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-[11px] text-slate-400">{s.label} · {s.sub}</p>
              </div>
            </div>
          </div>
        ))}
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        ...
        <div className="lg:col-span-2 space-y-3">
          <p className="text-xs font-bold uppercase tracking-widest text-slate-400">Quick Launch</p>
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            {tools.map(t => (
              <button
                key={t.key}
                onClick={() => onNavigate(t.key)}
                className="group flex items-center gap-3 rounded-2xl border border-white/10 bg-slate-900 p-4 text-left shadow-sm hover:border-violet-500/50 hover:bg-slate-800/80 transition-colors"
              >
                ...
                <ArrowRight size={14} className="flex-shrink-0 text-slate-600 group-hover:text-violet-400 transition-colors" />
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            ...
            <button onClick={() => onNavigate("activity")} className="text-[11px] font-medium text-violet-400 hover:text-violet-300">
              View all
            </button>
          </div>
          <div className="rounded-2xl border border-white/10 bg-slate-900 shadow-sm overflow-hidden">
            {recentActivity.length === 0 ? (
              <p className="px-4 py-6 text-center text-xs text-slate-500">No activity yet</p>
            ) : (
              <div className="divide-y divide-white/5">
                {recentActivity.slice(0, 6).map(a => (
                  <div key={a.id} className="px-4 py-2.5">
                    <p className={cn("truncate text-xs text-slate-200")}>
                      <span className="font-semibold">{a.entityType}</span>: {a.detail}
                    </p>
                    <p className="text-[10px] text-slate-500">by {a.actorName}</p>
                  </div>
                ))}
              </div>
            )}
