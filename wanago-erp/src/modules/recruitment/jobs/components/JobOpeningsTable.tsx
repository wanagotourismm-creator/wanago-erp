...
export function JobOpeningsTable({ jobs, loading, canManage, onView, onEdit, onDelete, onStatus }: Props) {
    ...
    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
      {jobs.map(job => (
        <div
          key={job.id}
          onClick={() => onView(job)}
          className="cursor-pointer rounded-2xl border border-border bg-card p-5 shadow-sm space-y-3 hover:border-primary/40 transition-colors"
        >
          ...
          {canManage && (
            <div className="flex items-center gap-2 pt-2 border-t border-border">
              <button onClick={(e) => { e.stopPropagation(); onEdit(job); }} className="flex items-center gap-1.5 rounded-lg px-2 py-1 text-xs font-medium text-foreground hover:bg-muted transition-colors">
                <Edit2 size={12} /> Edit
              </button>
              <button onClick={(e) => { e.stopPropagation(); onDelete(job); }} className="flex items-center gap-1.5 rounded-lg px-2 py-1 text-xs font-medium text-destructive hover:bg-destructive/10 transition-colors">
